...
export function LeadChat({ token, businessName }: { token: string; businessName: string }) {
  const [messages, setMessages] = useState<ChatEntry[]>([
    { role: "sally", content: `Hi, you've reached ${businessName}. I'm Sally, their assistant — they're on a job right now. What's going on?` },
  ]);
  ...
  function send() {
    ...
    startTransition(async () => {
      const result = await sendLeadMessage(token, text, conversationIdRef.current);
